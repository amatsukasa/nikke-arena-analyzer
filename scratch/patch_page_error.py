...
target_str = """  if (loading) return (
    <div className="flex h-screen items-center justify-center">
      <div className="animate-spin w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full" />
    </div>
  );"""

replacement_str = """  if (loading) return (
    <div className="flex h-screen items-center justify-center">
      <div className="animate-spin w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full" />
    </div>
  );
...
if target_str in code:
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(code)
    print("Success: Patched page.tsx")
else:
    # 改行コードの差異（CRLF vs LF）に対応
    target_str_lf = target_str.replace("\r\n", "\n")
    code_lf = code.replace("\r\n", "\n")
    if target_str_lf in code_lf:
        code_lf = code_lf.replace(target_str_lf, replacement_str.replace("\r\n", "\n"))
        # CRLF に戻して書き出す
        code_crlf = code_lf.replace("\n", "\r\n")
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(code_crlf)
        print("Success: Patched page.tsx (LF normalized)")
    else:
        print("Error: Target string not found in page.tsx")
