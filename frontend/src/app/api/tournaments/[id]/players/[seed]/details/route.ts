...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string; seed: string }> }) {
  try {
    const { id, seed } = await params;
    const token = request.cookies.get('auth_token')?.value;
    const res = await fetch(`${BACKEND_URL}/api/tournaments/${id}/players/${seed}/details`, {
      method: 'GET',
      headers: {
