...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string; charId: string }> }) {
  try {
    const { id, charId } = await params;
    const authHeader = request.headers.get('authorization');
    const token = authHeader?.startsWith('Bearer ') ? authHeader.slice(7) : request.cookies.get('auth_token')?.value;
    const query = request.nextUrl.searchParams.toString();
    const url = `${BACKEND_URL}/api/tournaments/${id}/dashboard/character/${charId}${query ? `?${query}` : ''}`;
