...
import { ChevronLeft, TrendingUp, Users, Trophy, Swords, User as UserIcon } from "lucide-react";
...
export default function CharacterDetailPage() {
  ...
  // 大会横断モード: クエリパラメータ tournaments=1,2,3 から復元
  const tournamentsParam = searchParams.get("tournaments");
  const selectedTournamentIds: number[] = tournamentsParam
    ? tournamentsParam.split(",").map(Number).filter(Boolean)
    : [Number(tournamentId)];
  const isCrossMode = selectedTournamentIds.length > 1 || (selectedTournamentIds.length === 1 && selectedTournamentIds[0] !== Number(tournamentId));
  ...
  useEffect(() => {
    if (!tournamentIdResolved) return;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [tournRes, charsRes] = await Promise.all([
          fetch(`/api/tournaments/${tournamentIdResolved}?t=${timestamp}`, { cache: 'no-store' }),
        ...
        // Stats: 大会横断モード対応
        const isSingleCurrent = selectedTournamentIds.length === 1 && selectedTournamentIds[0] === Number(tournamentId);
        let statsData;

        if (isSingleCurrent) {
          const statsRes = await fetch(`/api/tournaments/${tournamentIdResolved}/dashboard/stats?t=${timestamp}`, { cache: 'no-store' });
  ...
  if (!c) {
    return (
      <main className="p-6 md:p-12 max-w-4xl mx-auto">
        <p className="text-slate-400 text-center py-12">キャラクターが見つかりません (ID: {charId})</p>
        <div className="text-center">
          <Link href={`/tournament/${tournamentId}/dashboard`} className="text-blue-400 hover:text-blue-300 font-bold">
            ← ダッシュボードに戻る
          </Link>
        </div>
      </main>
  ...
  const usageData = stats?.character_usage?.find((x: any) => x.id === charId);
  ...
  const relatedTeams = stats?.team_usage?.filter((t: any) => t.character_ids.includes(charId)) || [];
    ...
    return (
      <div className="flex space-x-2">
        {displayChars.map((ch: any, i: number) => (
          <Link
            key={i}
            href={`/tournament/${tournamentId}/dashboard/character/${ch.id}${tournamentsParam ? `?tournaments=${tournamentsParam}` : ''}`}
  ...
  // ダッシュボードへのリンク（大会横断パラメータを維持）
  const dashboardUrl = `/tournament/${tournamentId}/dashboard`;
    ...
    <main className="p-6 md:p-12 max-w-4xl mx-auto space-y-8 pb-24">
      ...
      {isCrossMode && (
        <div className="bg-cyan-500/10 px-4 py-2 rounded-xl ring-1 ring-cyan-500/30 flex items-center space-x-2">
          <span className="text-[10px] font-black text-cyan-400">横断モード: {selectedTournamentIds.length}大会の統合データ</span>
        </div>
      )}
              ...
              {burst1Chars.map(ch => (
                ...
                >
                  ...
                  <div className="flex flex-col items-center">
                    <span className="text-[10px] text-slate-300 w-16 truncate text-center font-bold" title={ch.name}>{ch.name}</span>
          ...
          {[...relatedTeams]
            ...
            .map((team: any, idx: number) => {
              ...
              return (
                <div
                  key={idx}
                  onClick={() => router.push(`/tournament/${tournamentId}/dashboard?tab=matchups&team=${encodeURIComponent(team.canonical_id)}${tournamentsParam ? `&tournaments=${tournamentsParam}` : ''}`)}
